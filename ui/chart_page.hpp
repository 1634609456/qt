...
private:
    ...
    // 张力检测相关的成员变量
    QWidget *container_tension_widget_{};
    std::unique_ptr<webview::webview> tension_webview_;
    QWidget *chart_tension_widget_{};
    QJsonArray tension_datas_;
...
inline ChartPage::ChartPage(QWidget *parent)
    ...
    //       container_test_widget_)}
      ...
      container_tension_widget_(new QWidget()),
      tension_webview_(std::make_unique<webview::webview>(true, reinterpret_cast<HWND>(container_tension_widget_->winId()))),
      chart_tension_widget_{QWidget::createWindowContainer(
          QWindow::fromWinId(reinterpret_cast<WId>(reinterpret_cast<HWND>(tension_webview_->widget().value()))),
          container_tension_widget_)}
          {
    this->_init_content();
...
// ========== 新增：张力检测页面（ATM+张力图）- 基于原重复ATM代码修改，不删任何ATM内容 ==========
{
         // 用独立的张力WebView，不影响原ATMWebView
         tension_webview_->set_virtual_hostname("charts", "./res");
         tension_webview_->navigate("http://charts/tension-data.html"); // 独立的张力前端页面
 ...
 // ========== 关键修改：两个张力值都执行公式转换 ==========
        ...
        data_json["tensionData"] = QJsonArray{
            ...
            if (tension_datas_.size() > 1000 / sampling_time) {
                QString data_str{QJsonDocument(tension_datas_).toJson(QJsonDocument::Compact)};
                tension_webview_->eval("updateChartData(" + data_str.toStdString() + ");");
                tension_datas_ = QJsonArray();
            }
        });
        ...
        // 将张力页面添加到栈控件，对应Pivot的“张力检测”选项
        stack_->addWidget(chart_tension_widget_);
}
